...
if [[ `uname -s` == "Darwin" ]]
then
  if ! javac -classpath .:${SOARLIB}/swt-osx.jar:${SOARLIB}/sml.jar -sourcepath src src/edu/umich/tanksoar/TanksoarJ.java; then
    echo "Build failed."
    exit 1;
  fi
  jar cfm ${SOARLIB}/tanksoar.jar JarManifest-osx -C src .
else
  if ! javac -classpath .:${SOARLIB}/swt-motif.jar:${SOARLIB}/sml.jar -sourcepath src src/edu/umich/tanksoar/TanksoarJ.java; then
    echo "Build failed."
    exit 1;
  fi
