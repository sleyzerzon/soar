...
import javax.swing.JOptionPane;//TODO don't use swing, maybe. Is it bad to mix swing and swt?
import java.io.*;
import java.util.*;

/**
 * The model behind a TankSoar simulation, this maintains the map and keeps track
 * of the agents in the simulation, notifying them when they need to calculate moves
...
public class TankSoarJControl extends SimulationControl implements Kernel.UpdateEventInterface, Kernel.SystemEventInterface, SimulationControlListener, Runnable{
  ...
  private String soarLibraryName = "SoarKernelSML";
	/** The score at which one <code>Tank</code> is decided to have won the game. */
	...
	/** An <code>ArrayList</code> of all the missiles that have been shot and are
	 * currently 'flying' in the simulation. */
  ...
  private HashMap decisions = new HashMap();
    ...
    //TankSoarLogger.log("\tTankSoarJControl::constructor creating kernel inprocess");
    //kernel = Kernel.CreateKernelInCurrentThread(soarLibraryName, false, 12121);  
    ...
    //FIXME //TODO duplicates a constant that's in the ControlPanel class. get legit access instead
    String defaultMapsPath = "tanksoar" + File.separator + "maps"; // this is where we expect maps to be located
	...
	/**
	 * Cretaes a new instance of <code>TankSoarJControl</code> using the map specified
	 * at <code>infile</code> as the map to be used for the simulation.
	 * @param infile Expected to be a properly formatted map file for TankSoar.
    ...
    //TankSoarLogger.log("\tAbout to load map: " + infile.getAbsolutePath());
    loadMap(infile);
	}
  
  private void checkKernelStatus()
  {
    if(kernel == null)//this doesn't really happen with our SWIG wrapped stuff.  Failure will yeild a non-null reference
      TankSoarLogger.log("\tTankSoarJControl::constructor kernel was null after supposed creation.");
    else
    {
      ...
      if(kernel.HadError())
      {
        TankSoarLogger.log("\tError during kernel creation, but instance isn't null"); 
        JOptionPane.showMessageDialog(null, "Fata error during kernel initialization .", "Fatal Error",
  ...
  private void activateDetailedKernelLogging(boolean should)
  {
    ...
    if(!kernel.GetLastCommandLineResult())
      ...
      TankSoarLogger.log("Succeeded in turning on detailed trace logging.");
  }
  ...
  private void registerForSoarStopEvent()
  {
    ...
    {
      TankSoarLogger.log("Failed to successfully register for Soar kernel stop event.");
  ...
  private void registerForSoarStartEvent()
  {
    ...
    {
      TankSoarLogger.log("Failed to successfully register for Soar kernel start event.");
      TankSoarLogger.log("\t" + kernel.GetLastErrorDescription());
    }    
  }
	 ...
	 * <p> The first line is a single character, either a 0 or a 1. If it is a
	 * 1, the simulation will use deterministically random numbers for
	 * missile placement, setting the random parameter in the <code>Tanks</code>'
	 ...
	 * on the current system time.</p>
	 * <p>The next <code>MapHeight</code> lines of the file must be of width
	...
	public void loadMap(File infile)
  {
    ...
    TankSoarLogger.log("\tMap file loaded.");        
		try
    {            
      //determine if map is a Tcl map or Java one.  Tcl maps have strings as first meta data chars,
      //JavaTankSoar maps have a digit as the first meta data character //TODO actually do this
		...
		try
    {
		...
		placeAllTanks();
		fireNewMapNotification((setRandoms ? new String("Deterministically random setting will be used to run simulation.\nRandoms seeded"):null));
  }
  ...
  protected void loadJavaMap()
  {
	 ...
	 * Creates a new open map, resets all the agents, and fires a new map notification
	 * for the listeners.
	 * @param message The message associated with the exception or error, if there is any.
	 */
	...
	/**
	 * Creates an empty, obstacle-free map for the simulation. Will put in a recharger and
	 * a health square.
	 */
	...
	/**
	 * Initializes all the boundaries of the map to be obstacles so no <code>Tank</code>s
	...
	/**
	 * Fills all non-null squares in the map with <code>TSEmpty</code>s, so they are enterable
	 * by <code>Tank</code>s. Also randomly places a <code>HealthSquare</code> and an
	 * <code>EnergySquare</code>.
	 */
	...
	/** <code>true</code> if there has been a <code>HealthSquare</code> placed in
	 * this simulation. */
	private boolean healthCreated = false;
	/** <code>true</code> if there has been an <code>EnergySquare</code> placed in
	 * this simulation. */
	...
	/**
	 * Randomly places an <code>EnergySquare</code> in the simulation using the
	 * <code>findEmptyLocation()</code> method. If an <code>EnergySquare</code>
	 * has already been placed, no action is taken.
	 * @see TankSoarJControl#findEmptyLocation()
	 */
	...
	/**
	 * Places one <code>MissileBucket</code> at a random <code>TSEmpty</code> square in the simulation.
	 * Increments the ivar storing the number of <code>MissileBucket</code>s in the simulation.
	...
	/**
	 * Notification called when a <code>MissileBucket</code> has been picked up by a <code>Tank</code>,
	 * called by the <code>MissileBucket</code> itself. Decrements the ivar storing the number of
	 * <code>MissileBucket</code>s that are in the simulation.
	 * @param pickedUp The <code>MissileBucket</code> that has been picked up.
	 */
	...
	/**
	 * <p>Constructs a new map for the simulation. It is not random, per-say, but it is
	 * a gigantic spiral whose spiralness may be broken by a <code>HealthSquare</code>
	 * or an <code>EnergySquare</code>.
	 * {@inheritDoc}
	 */
	...
	/**
	 * Places all the <code>Tank</code>s in the simulation at random places in the map.
	 * If the random seed has been set, should work the same way every time.
	 */
	...
	/**
	 * Prints the the statistics for all the <code>Tank</code>s in the simulation.
	 * {@inheritDoc}
	 */
			...
			for(int x = 0; x < MapHeight; x++){
				if(myMap[x][y] instanceof TSEmpty){
					if(((EnterableSquare)myMap[x][y]).containsAgent()){
						so.print('+');
					} else if(((EnterableSquare)myMap[x][y]).containsMissiles()){
						so.print('M');
	 ...
	 * <code>Location</code>, and an <code>int</code> specifying the <code>Tank</code>'s
	 * direction, used for recalling the original <code>Location</code>s and
	 * directions of the <code>Tank</code>s upon creation.
	 */
	private static class TankLocationTriple{
		...
		/**
		 * Constructs a <code>TankLocationTriple</code> with the specified parameters.
		 * @param t The <code>Tank</code> to be stored.
		 * @param l The <code>Tank</code>'s <code>Location</code>.
		 * @param d The direction the <code>Tank</code> faces, as defined in <code>Tank</code>.
		 */
	...
	/** An <code>ArrayList</code> containing all the original locations and directions
	 * of the <code>Tank</code>s in the simulation. Made up of <code>TankLocationTriple</code>s.
	 */
	private ArrayList originalLocations = new ArrayList(8);
	
	/**
	 * Resets the map and all the <code>Tank</code>s in it to the positions at which
	 * they were originally located, with their correct directions.
  ...
  protected void finalize()
  {
    if(kernel != null) 
    {
      ...
      while(agentItr.hasNext())
      {
        ...
        {
          TankSoarLogger.log("TankSoarJControl::finalize: tried to destruct something that wasn't a Tank.");
        }
      }
    }
  }
	/**
	 * Notifies all <code>Tank</code>s that there has been a change in the number of
	 * agents, allowing them to recalculate their input-link sensors.
	 * {@inheritDoc}
	 */
...
TankSoarLogger.log("\t\tTankSoarJControl::destroyAgent called...");
	...
	public void destroyAllAgents()
  {
	...
	/** <code>true</code> if the random number generators for a simulation have been seeded. */
	private boolean randomSeedsSet = false;
	
	/**
	 * Using calls to <code>seedRandomNumber()</code>, makes the running of the simulation
	 * deterministically random. Sets the random seeds for <code>MissileBucket</code>s,
	 * <code>Tank</code>s, and this <code>TankSoarJControl</code>.
	 */
	...
	/**
	 * Using calls to <code>unseedRandomNumber()</code>, makes the running of the simulation
	 * non-deterministically random. Unsets the random seeds for <code>MissileBucket</code>s,
	 * <code>Tank</code>s, and this <code>TankSoarJControl</code>.
	 */
	...
	/** The number of <code>MissileBucket</code>s in the simulation. That is, the number of
	 * positions at which a <code>Tank</code> can pick up missiles. */
	private int missileBucketCount = 0;
	/** The maximum number of <code>MissileBucket</code>s that can exist in the simulation
	 * at one time. */
	private static final int MaxMissileBucketCount = 3;
	/** The number of steps that have elapsed since a <code>MissileBucket</code> was placed. */
  ...
  /**
	 ...
	 * coordinate is outside the indices of the map, returns a <code>TSWall</code>.
	 */
	...
	/**
	 * In <code>TankSoarJControl</code>, also calculates whether and where an new
	 * <code>MissileBucket</code>s should be placed, based on the number of turns
	 * since a <code>MissileBucket</code> had been placed and a random double from
	 * <code>MissileBucket</code>.  Too, evaluates missiles flying and <code>Tank</code>
	 * collisions.
	 * {@inheritDoc}
	 */
  ...
  {   
    ...
    //now all decisions have been made, and should be waiting on the output link
    TankSoarLogger.log("TankSoarJControl: Make decision here....");

    for(Iterator iter = myAgents.listIterator(); iter.hasNext();)
    {
      Tank curr = ((Tank)iter.next());
      //This will set the tank's internal state, so that it matches up with the move
      //that it just decided to do
      decisions.put(curr.getName(), (TankOutputInfo)curr.makeDecision());
    }
    
    //evaluate all of the decisions that were just made - that is, enact whatever 
    ...
    for(Iterator iter = myAgents.listIterator();iter.hasNext();)
    {
      try
      {
        ...
        TankOutputInfo curDecision = (TankOutputInfo)decisions.get(curAgent.getName());
        if(curDecision != null)
          evaluateDecision(curDecision, curAgent);
      }
      catch(ClassCastException e)
      {
        TankSoarLogger.log("Something that wasn't a TankOutputInfo ended up in the decisions HashMap, or something that's not a SoarAgent ended up in the list of agents....");
      }
    }
    ...
    {
      if((double)turnsSinceMissile/50.0 > MissileBucket.getNextRandomDouble())
        placeMissileBucket();
      else
        ++turnsSinceMissile;
    }
	...
	/**
	 * Iterates over all the missiles flying in the simulation, calling their <code>fly()</code>
	 * methods and checking to see if they successfully flew. Fires notifications of the changes
	 * to the new and old <code>Location</code>s they cover.
	 */
	private void evaluateMissiles(){
		...
		while(mIter.hasNext()){
			...
			boolean flew = fm.fly();
			((EnterableSquare)myMap[oldLoc.getX()][oldLoc.getY()]).removeOccupant(fm);
			if(flew){
	...
	/**
	 * Given a <code>Tank</code> and its decision, in the form of a <code>TankOutputInfo</code>,
	 * evaluates the decision, setting the <code>Tank</code>'s position using <code>setTankPosition(Tank,
	 * Location)</code>. Also, if the decision indicates missiles ought to be shot, checks to
	 * make sure <code>sa</code> can fire missiles, then places <code>FlyingMissile</code>s in the
	 * simulation. Calls <code>Tank.turnFinished(boolean)</code> to tell the <code>Tank</code> that
	 * it is done and whether its move was successful.
	 * @see TankSoarJControl#setTankPosition(Tank, Location)
	 * {@inheritDoc}
	 */
	protected void evaluateDecision(Object decision, SoarAgent sa)
  {
    ...
    {           
			if(tDecision.move.direction.startsWith("l")){
				adjacent = adjacent.getAdjacent(Tank.getTurnDirection(direction, Tank.TURNLEFT));
			} else if(tDecision.move.direction.startsWith("r")){
				adjacent = adjacent.getAdjacent(Tank.getTurnDirection(direction, Tank.TURNRIGHT));
				...
				adjacent = adjacent.getAdjacent(Tank.getOppositeDirection(direction));
			}
		}
        ...
        TankSoarLogger.log("about to fire changed location notification");
				fireLocationChangedNotification(loc);
			}
		}
	...
	/**
	 * Gives to the caller an array of all the <code>FlyingMissile</code>s currently
	 * in the "air" in the simulation.
	...
	/**
	 * Sets the given <code>Tank</code>'s position to be that specified by the <code>
	 * Location</code>. Checks to make sure the square is enterable, and, if it is
	 * calls the <code>Tank</code>'s <code>setLocation(Location)</code> method.
	 * Otherwise, crashes the <code>Tank</code> and returns <code>false</code>.
	 * @param t The <code>Tank</code> whose position is being set.
	 * @param l The <code>Location</code> at which to place the <code>Tank</code>.
	 * @return <code>true</code> if the <code>Location</code> specified contained an
	 * <code>EnterableSquare</code> so that the <code>Tank</code> could be positioned
	 * there, <code>false</code> otherwise.
	 */
	private boolean setTankPosition(Tank t, Location l){
		Location oldLoc = t.getLocation();
		if(oldLoc.equals(l) || getLocationContents(l) instanceof EnterableSquare){
			t.setLocation(l);
			return true;
		}
		t.crashed();
	...
	/**
	 * For the specified <code>Tank</code>'s input link, represented by <code>sensors</code>,
	 * updates the <code>rwaves</code>, <code>smell</code>, and <code>sound</code> sensors
	 * (rather than simply the radar waves). Iterates over all the other <code>Tank</code>s
	 * in the simulation, checking each of their radars and filling the sensors. Infeffecient?
	 * Yes. But there are only a maximum of seven tanks anyway.
	 * @param sensors The <code>TankInputInfo</code> representing toFill's input link.
	 * @param toFill The <code>Tank</code> whose input-link sensors are being updated.
	 */
	public void fillRwavesSensor(TankInputInfo sensors, Tank toFill){
		...
		while(iter.hasNext()){
			Tank t = (Tank)iter.next();
			if(t != toFill){
				if((distance = (Math.abs(toFill.getLocation().getX() - t.getLocation().getX())
						+ Math.abs(toFill.getLocation().getY() - t.getLocation().getY()))) < leastDistance){
					...
					if(t.movedLastTurn() && leastDistance < Tank.MaxHear){
						...
						soundDirection = toFill.getDirectionFromTank(t.getLocation());
					}
				}
				Location[] locs = t.getRadarLocations();
				for(int i = 0; i < locs.length; ++i){
					if(locs[i] != null && locs[i].equals(toFill.getLocation())){
						int direction = toFill.getDirectionFromTank(t.getLocation());
						switch(direction){
						case(Tank.FORWARD):{ sensors.rwaves.forward = TankInputInfo.yes; break; }
						case(Tank.RIGHT):{ sensors.rwaves.right = TankInputInfo.yes; break; }
						case(Tank.LEFT):{ sensors.rwaves.left = TankInputInfo.yes; break; }
						case(Tank.BACKWARD):{ sensors.rwaves.backward = TankInputInfo.yes; break; }
						}
					}
				}
			}
		}
		if(soundDirection != -1){ soundSensorFill(soundDirection, sensors); }
	...
	/**
	 * Given that a <code>Tank</code> ought to hear a sound (another <code>Tank</code>
	 * moved and was close), sets the specified <code>TankInputInfo</code> to have
	 * <code>sensors.sound</code> parameters correctly filled with the direction from
	 * which the sound has come.
	 * @param direction The direction (specified in <code>Tank</code>) from which the sound comes.
	 * @param sensors The <code>TankInputInfo</code> whose parameters are being updated.
	 */
	private void soundSensorFill(int direction, TankInputInfo sensors){
		switch(direction){
		case(Tank.FORWARD): sensors.sound = "forward"; break;
		case(Tank.RIGHT): sensors.sound = "right"; break;
		case(Tank.LEFT): sensors.sound = "left"; break;
		case(Tank.BACKWARD): sensors.sound = "backward"; break;
		}
	}
	
	/**
	 * Iterates over the <code>FlyingMissile</code>s in the simulation, checking to
	 * see if any are flying directly toward the specified <code>Tank</code>, filling
	 * the <code>TankInputInfo</code> specified's <code>incoming.forward, backward, left,</code>
	 * and <code>right</code> sensors if any missiles are on the same x or
	 * y coordinate the <code>Tank</code> is and flying at the <code>Tank</code>.
	 * @param sensors The <code>TankInputInfo</code> that will be given the <code>Tank</code>
	 * when it must make a decision whose sensors need to be updated.
	 * @param toFill The <code>Tank</code> whose input sensors are represented by
	 * <code>sensors</code>.
	 */
	public void fillIncomingSensor(TankInputInfo sensors, Tank toFill){
		...
		while(iter.hasNext()){
			...
			int tankX = toFill.getLocation().getX(), tankY = toFill.getLocation().getY();
			...
			case(Tank.FORWARD):{ sensors.incoming.forward = TankInputInfo.yes; break; }
			case(Tank.BACKWARD):{ sensors.incoming.backward = TankInputInfo.yes; break; }
			case(Tank.RIGHT):{ sensors.incoming.right = TankInputInfo.yes; break; }
			case(Tank.LEFT):{ sensors.incoming.left = TankInputInfo.yes; break; }
			}
		}
	}
	...
	/**
	 * Fills the provided <code>TankInputInfo</code> with the sensory information
	 * available to the provided <code>Tank</code>--that is, its radar, sound,
	 * smell, and incoming sensors via calls to <code>fillRwavesSensor(TankInputInfo,
	 * Tank)</code> and <code>fillIncomingSensor(TankInputInfo, Tank)</code>.
	 * @param sensors The <code>TankInputInfo</code> representing the information
	 * available to <code>toFill</code>.
	 * @param toFill The <code>Tank</code> whose input-link sensors are being filled.
	 */
	 ...
	 * @param winner The <code>Tank</code> that has won the simulation.
	 */
	 ...
	 * @return The <code>HumanTankControl</code> created if creation was successful, <code>null</code>
	 * otherwise.
	 */
	 ...
	 * @return The <code>Tank</code> that has been created, if creation was successful,
	...
	/**
	 * This method will try to create the <code>Tank</code> with the given color.
	 * If a file ending with .soar is passed, will place the <code>Tank</code> randomly,
	 * though a .stank file can be passed as well, configuring the <code>Tank</code>'s
	 * starting position, color, and direction.
	 * {@inheritDoc}
	 */
...
try{
			  ...
			  Tank t = createTank(colorName);
				if(t != null)
				{
				  t.setKernel(kernel);
				    ...
				    TankSoarLogger.log("\t't' was null after createTank call!");
				return(t);				
		  }//try
		  catch(NullPointerException e)
		  {
		    TankSoarLogger.log("\tNull ptr while trying to load a new agent..." + e.getMessage());
		    return null;
		  }
      ...
      return newTank;
		...
		//else
		//  TankSoarLogger.log("TankSoarJControl::loadAgent did not get filename ending in stank or soar, returning.");
}
catch(NullPointerException e)
{
  TankSoarLogger.log("\tCaught a null ptr exception..." + e.getMessage());
}
    return(null);
	}
	
	/**
	 * Creates a <code>Tank</code> using the specified *.stank file, which can deterimine the
	 * <code>Tank</code>'s <code>location</code>, direction, color, and will
	 * deterimine its .soar file.
	 * <p>A .stank file must have a line specifying the Soar file whose productions will
	 * be used to control the agent. This takes the form <p>file <i>file_name</i>.soar</p>
	 * <p>It may also include lines specifying location, color, and direction, as follows:
	 * <ul><li>location <i>x-coordinate y-coordinate</i></li><li>direction <i>north/east/south/west</i>
	 * </li><li>color <i>red/green/black...</i></li></ul>
			...
			while((line = bIn.readLine()) != null){
				...
				if(line.startsWith("location")){
					...
					try{
						...
						switch(dir){
						case('n'): direction = Tank.NORTH; break;
						case('s'): direction = Tank.SOUTH; break;
						case('w'): direction = Tank.WEST; break;
						case('e'): direction = Tank.EAST; break;
						}
					} catch(ArrayIndexOutOfBoundsException e){
						direction = -1;
					}
	...
	/**
	 * Creates a <code>Tank</code> at random location with color specified in the simulation.
	 * If color has been used, random color will be used for <code>Tank</code>.
	 * @param colorName The String name of the color to be used to create this <code>Tank</code>.
	 * @return The <code>Tank</code> that has been created, if creation was successful,
	 * <code>null</code> otherwise.
	 */
	...
	/**
	 * Creates a <code>Tank</code> at specifed location with random color in the simulation.
	 * If the Location specifed is not empty, <code>Tank</code> will be placed randomly.
	 * @param loc The location desired at which the <code>Tank</code> will be created.
	 * @return The <code>Tank</code> that has been created, if creation was successful,
	 * <code>null</code> otherwise.
	 */
	...
	/**
	 * Creates a <code>Tank</code> at specified location with specified color in the simulation.
	 * If the color has already been used by another agent in the simulation or the <code>
	 * Location</code> specified is not empty, they will be (respectively) random.
	 * @param colorName The <code>String</code> name of the color requested.
	 * @param loc The <code>Location</code> at which it is desired to create the
	 * <code>Tank</code>.
	 * @return The <code>Tank</code> that has been created, if creation was successful,
	 * <code>null</code> otherwise.
	 */
	...
	/**
	 * Creates a <code>Tank</code> at specified location with specified color and
	 * direction in the simulation. If the color has already been used by another
	 * agent in the simulation or the <code>Location</code>
	 * specified is not empty, they will be (respectively) random.
	 * @param colorName The <code>String</code> name of the color requested.
	 * @param loc The <code>Location</code> at which it is desired to create the
	 * <code>Tank</code>.
	 * @param direction The desired direction (as defined in <code>Tank</code>) of the
	 * <code>Tank</code> being created. If <code>direction < 0</code> will be random
	 * in [0, 3]
	 * @return The <code>Tank</code> that has been created, if creation was successful,
	 * <code>null</code> otherwise.
	 */
	public Tank createTank(String colorName, Location loc, int direction){
		direction = (direction < 0 ? (int)(getNextRandomDouble()*4):direction%4);
		boolean empty = (loc != null ? emptyLocation(loc.getX(), loc.getY()): false);
		if(myEcs.colorAvailable(colorName)){
			if(empty){
				return(createTankColorLocationDirection(colorName, loc, direction, false));
			}
	...
	/**
	 * Creates a <code>Tank</code> with the first available color at a random location in
	 * the simulation.
	 * @param human If <code>true</code>, will create a <code>HumanTankControl</code>, a human-controlled
	 * <code>Tank</code>.
	 * @return The <code>Tank</code> that has been created, if creation was successful,
	 * <code>null</code> otherwise.
	 */
	...
	/**
	 * Creates a <code>Tank</code> at the specified <code>Location</code> in the simulation. Since this is
	 * a private method, assumes the <code>Location</code> is available.
	 * @param loc The <code>Location</code> at which to create the <code>Tank</code>.
	 * @param human If <code>true</code>, will create a <code>HumanTankControl</code>, a human-controlled
	 * <code>Tank</code>.
	 * @return The <code>Tank</code> that has been created, if creation was successful,
	 * <code>null</code> otherwise.
	...
	private Tank createTankLocation(Location loc, boolean human){
		return(createTankLocation(loc, (int)(getNextRandomDouble()*4), human));
	}
	
	/**
	 * Creates a <code>Tank</code> at the specified <code>Location</code> in the simulation. Since this is
	 * a private method, assumes the <code>Location</code> is available and that the
	 * direction specified is valid (in the range [0, 3]).
	 * @param loc The <code>Location</code> at which to create the <code>Tank</code>.
	 * @param direction The direction (as defined in <code>Tank</code>) of the
	...
	private Tank createTankLocation(Location loc, int direction, boolean human){
		String[] colors = myEcs.colorsAvailable();
		if(colors.length > 0){
			return(createTankColorLocationDirection(colors[0], loc, direction, human));
		}
		return null;
	}
	
	/**
	 * Creates a <code>Tank</code> of the specified color in the simulation. Since this is
	 * a private method, assumes the color specified is available.
	 * @param colorName The name of the color to use as the color of the <code>Tank</code> being
	 * created.
	 * @param human If <code>true</code>, will create a <code>HumanTankControl</code>, a human-controlled
	 * <code>Tank</code>.
	 * @return The <code>Tank</code> that has been created, if creation was successful,
	...
	/**
	 * Creates a <code>Tank</code> of the specified color in the simulation. Since this is
	 * a private method, assumes the color specified is available and that the direction
	 * is valid (i.e. in the range [0, 3]).
	 * @param colorName The name of the color to use as the color of the <code>Tank</code> being
	 * created.
	 * @param direction The direction (as defined in <code>Tank</code>) of the
	 * <code>Tank</code> being created.
	...
	private Tank createTankColor(String colorName, int direction, boolean human){
		Location loc = findEmptyLocation();
		return(createTankColorLocationDirection(colorName, loc, direction, human));
	}
	
	/**
	 * Creates a <code>Tank</code> at the specified <code>Location</code> and with the
	 * specified color in the simulation. Since this is a private method, assumes the color
	 * and <code>Location</code> specified are available.
	 * @param colorName The name of the color to use as the color of the <code>Tank</code> being
	 * created.
	 * @param loc The <code>Location</code> at which to create the <code>Tank</code>.
	 * @param human If <code>true</code>, will create a <code>HumanTankControl</code>, a human-controlled
	 ...
	 * <code>null</code> otherwise.
	 */
	private Tank createTankColorLocation(String colorName, Location loc, boolean human){
		return(createTankColorLocationDirection(colorName, loc, (int)(getNextRandomDouble()*4), human));
	}
	 ...
	 * created.
	 * @param loc The <code>Location</code> at which to create the <code>Tank</code>.
	 * @param direction The <code>int</code> value of the direction to create this <code>Tank</code>
	 * pointing in, as defined in <code>Tank</code>.
	 * @param human If <code>true</code>, will create a <code>HumanTankControl</code>, a human-controlled
	 * <code>Tank</code>.
	 * @return The <code>Tank</code> that has been created, if creation was successful,
	 * <code>null</code> otherwise.
	 */
	private Tank createTankColorLocationDirection(String colorName, Location loc, int direction, boolean human){
		Tank t = null;
		if(human){
	...
	/**
	 * Notifes all the <code>Tank</code>s in the simulation that another agent
	 * has been created or destroyed so that the <code>Tank</code> can recalculate
	 * its sensors (uses <code>Tank.allTanksFinished()</code>).
	...
	/**
	 * Called every time a <code>Tank</code> is created, clears the collection of original location
	 * triples, then fills it again with the <code>Tank</code>s and their positions and directions
	 * when this is called.
	 */
	private void setOriginalLocations(){
		...
		while(agentIterator.hasNext()){
			...
			originalLocations.add(new TankLocationTriple((Tank)a, a.getLocation(), ((Tank)a).getDirection()));
		}
	}
	
	/**
	 * An <code>ArrayList</code> that contains all the <code>Location</code>s that have been
	 * entered during a step of the simulation by a <code>Tank</code> when there was already
	 * a <code>SoarAgent</code> occupying the <code>Location</code>.
	 * @see TankSoarJControl#addCollisionLocation(Location)
	 */
	private ArrayList collidedEnterableLocations = null;
	/**
	 * An <code>ArrayList</code> that contains all the <code>Location</code>s that <code>
	 * Tank</code>s have exited during a turn, so that in undoing moves, these <code>
	...
	/** Parameter that tells whether collisions are being evaluated so calls to add a
	 * collided location can be ignored. */
	private boolean evaluatingCollisions = false;
	
	/**
	 * Adds a <code>Location</code> to the collection of <code>Location</code>s that have
	 * been exited by <code>Tank</code>s on a given turn. If this method is called
	 * when the simulation is evaluating <code>Tank</code> collisions, the method
	 * returns because it is simply a <code>Tank</code> performing an undo, and it
	 * is unnecessary to keep track of <code>Location</code>s exited during the undo.
	 * @param loc The <code>Location</code> exited by a <code>Tank</code>
	 */
	public void addExitedLocation(Location loc){
		if(exitedEnterableLocations == null) exitedEnterableLocations = new ArrayList();
		if(evaluatingCollisions) return; /* We don't care because we're already doing undo */
	...
	/**
	 * Adds a <code>Location</code> to the collection of <code>Location</code>s that have
	 * been entered by a <code>Tank</code> when there was already an agent at the <code>
	 * Location</code> on a given turn. If, when this method is called, the simulation
	 * is evaluating the collisions, this method is ignored because all <code>Tank</code>s
	 * have finished moving and it is simply an undo operation that causes the collision.
	 * @param loc The <code>Location</code> that two or more <code>SoarAgent</code>s have
	 * occupied for some part of evaluating a turn in the simulation.
	 */
	public void addCollisionLocation(Location loc){
		if(collidedEnterableLocations == null) collidedEnterableLocations = new ArrayList();
	 ...
	 * calling <code>checkCrossovers(Iterator)</code> and <code>undoMoves(Iterator, boolean)</code>
	 * on all the <code>Location</code>s that have been entered and exited by <code>Tank</code>s
	 * in a given turn. Also clears the containers of exited and collided <code>Location</code>s.
	...
	private void evaluateCollisions(){
		...
		if(collidedEnterableLocations == null){
			evaluatingCollisions = false; return;
		}
	...
	/**
	 * Iterates over the given <code>Location</code>s to check for collisions, undoing moves
	 * if there are collisions--for example, if more than one <code>Tank</code> resides at the
	 * given <code>Location</code>.
	 * @param iter An <code>Iterator</code> that contains the <code>Location</code>s whose
	 * contents will be queried to check for multiple agents.
	 * @param checkingExits <code>true</code> if the function is checking the exited
	 * <code>Location</code>s, which causes <code>Tank</code>s that have already undone moves not
	 * to be crashed into again.
	 */
			...
			if(numAgents > 1){
				...
				for(int i = 0; i < occupants.length; ++i){
					if(occupants[i] instanceof Tank){
						if(!checkingExits || !((Tank)occupants[i]).getLocationPrevious().equals(((Tank)occupants[i]).getLocation())){
	...
	/**
	 * Iterates over the gives <code>Location</code>s to check for collisions. Checks
	 * the contents of each <code>Location</code> for <code>Tank</code>s, and, if a
	 * <code>Tank</code> exists at the <code>Location</code>, checks the previous
	 * position of the <code>Tank</code> to see if there is a <code>Tank</code> there.
	 * If there is, checks the <code>Tank</code> in the previous position's previous
	 * <code>Location</code>; if it is the same as the first <code>Tank</code>'s current
	 * <code>Location</code>, undoes both their moves and tells them that they have
	 * crashed.
	 * @param collidedEnterIter An <code>Iterator</code> that contains the <code>Location</code>s
	 * whose contents will be queried.
	 */
	private void checkCrossovers(Iterator collidedEnterIter){
		while(collidedEnterIter.hasNext()){
			...
			for(int i = 0; i < occupants.length; ++i){
				if(occupants[i] instanceof Tank){
					Location previous = ((Tank)occupants[i]).getLocationPrevious();
					...
					if(previousSquare.getNumAgents() > 0){
						...
						for(int j = 0; j < prevOccupants.length; ++j){
							if(prevOccupants[j] instanceof Tank && !prevOccupants[j].equals(occupants[i]) && ((Tank)prevOccupants[j]).getLocationPrevious().equals(loc)){
								((Tank)occupants[i]).undoLastMove();
								((Tank)occupants[i]).crashed();
	...
	/** An <code>ArrayList</code> of <code>Tank</code>s that have been killed during a
	...
	/**
	 * Resurrects all the <code>Tank</code>s in the simulation that need resurrecting
	 * because they've been killed by finding them empty locations in the simulation
	 * and placing them there via <code>Tank.resurrect(Location)</code>.
	 */
	private void resurrectTanks(){
		if(tanksToResurrect == null) return;
	 ...
	 * <code>Tank</code> at a random <code>Location</code> in the simulation.
	 * @param killed The <code>Tank</code> whose health has gone to zero.
	 */
	...
	public void seedRandomNumber(){
		if(myRandom == null) myRandom = new Random(314);
		else myRandom.setSeed(314);
	}
	 ...
	 * this <code>TankSoarJControl</code>'s random number generator. If the random number
	...
	/**
	 * Gives caller a random empty location in the simulation's map, random defined as
	 * in <code>getNextRandomDouble()</code>. Note that if all locations in the map are
	 * full, will freeze simulation.
	...
	/**
	 * Tells caller whether the location at the specified coordinates is an TSEmpty location.
	 * @param x The x-coordinate of the location being queried.
	 * @param y The y-coordinate of the location being queried.
	...
	public boolean emptyLocation(int x, int y){
		try{
			return(myMap[x][y] instanceof TSEmpty &&
					!((TSEmpty)myMap[x][y]).containsAgent() && !((TSEmpty)myMap[x][y]).containsMissiles());
   ...
   * @param created As convenience, the tank created is passed with this method.
   */
  ...
  /**
   * Method defined to, if the simulation is not running, start the simulation (in a continuous loop).
   */
  public void runSimulation()
  {
    ...
    TankSoarLogger.log("TankSoarJControl::runSimulation() called....");
      ...
      while(running)
      {
//TankSoarLogger.log("runSimulation is going to call 'runAllTanks' right now");
        runAllTanks();
	...
	/**
	 * Fires a notification that the simulation has ended to all the <code>SimulationControlListener</code>s,
	 * but with the message that a <code>Tank</code> has won.
	...
	private void fireTankWonNotification(Tank winner){
		...
		while(iter.hasNext()){
			((SimulationControlListener)iter.next()).simEnded(winner.getColorName() + " tank has won.");
		}
	}

	/**
	 * Clears all the flying missiles in the simulation.
	 * {@inheritDoc}
	 */
...
//		TankSoarLogger.log("Soar 'start' event noticed by environment.");
  ...
  {
    TankSoarLogger.log("The call to the start event was instigated by " + caller);
    singleStep();
  }
