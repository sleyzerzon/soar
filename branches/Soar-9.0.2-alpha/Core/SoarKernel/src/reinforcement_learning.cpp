...
************************************************/
#include <sstream> //used in constructing production name below
...
production *build_production(agent *thisAgent, preference *pref)
{
	...
	std::string sprod_name = "";
	// increment counter until find a unique rule name
	while(true) {
		// convert number to string (we'll extract the string from this stream in a moment)
		std::ostringstream num; //recreate this each time through loop so numbers don't get appended to eachother
		num << thisAgent->RL_count;
		++thisAgent->RL_count; // increment the counter

		// build prod name as std::string (to avoid fixed buffer sizes)
		sprod_name = "RL-" + num.str() + "*" + pref->inst->prod->name->sc.name;

		// if name is unique, break out of loop
		if(find_sym_constant(thisAgent, sprod_name.c_str()) == NIL) { break; }
	}

	// make prod_name constant
	prod_name = make_sym_constant (thisAgent, sprod_name.c_str());
