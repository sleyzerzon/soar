...
public:
	...
	void get_slots(std::vector<std::string> &inputs, std::vector<std::string> &outputs) const {
		...
		inputs.push_back("lvolts");
		inputs.push_back("rvolts");
		splntr->append_slots(inputs);
		splntr->append_slots(outputs);
		for (i = blocks.begin(); i != blocks.end(); ++i) {
			(**i).append_slots(inputs);
			(**i).append_slots(outputs);
		}
	}
