...
void rl_set_qvalue(agent *a, production *p, double newval);
 ...
 Symbol *rl_build_template_instantiation( agent *my_agent, instantiation *my_template_instance, struct token_struct *tok, wme *w )
{
	...
	// set initial expected reward values
	{
		...
		new_production->qvalue = init_value;
	}
	...
	if ( add_production_to_rete( my_agent, new_production, cond_top, NULL, FALSE, TRUE ) == DUPLICATE_PRODUCTION )
	{
		...
		my_agent->dp->register_rule(new_production, new_production->qvalue);
	}
...
void rl_set_qvalue(agent *a, production *p, double newval) {
	preference *pref;
	instantiation *i;
	Symbol *valsym = rhs_value_to_symbol( p->action_list->referent );
	double oldval = get_number_from_symbol( valsym );
	
	if ( a->sysparams[ TRACE_RL_SYSPARAM ] ) {
		std::stringstream ss;
		ss << "updating RL rule " << p->name->sc.name << " from " << oldval << " to " << newval << std::endl;
		print( a, const_cast<char *>( ss.str().c_str() ) );
		xml_generate_message( a, const_cast<char *>( ss.str().c_str() ) );
	}

	// Change value of rule
	symbol_remove_ref( a, valsym );
	p->action_list->referent = symbol_to_rhs_value( make_float_constant( a, newval ) );
	p->rl_update_count += 1;

	// Change value of preferences generated by current instantiations of this rule
	for ( i = p->instantiations; i; i = i->next ) {
		for ( pref = i->preferences_generated; pref; pref = pref->inst_next ) {
			symbol_remove_ref( a, pref->referent );
			pref->referent = make_float_constant( a, newval );
		}
	}
}

void rl_update( agent *a, Symbol *g, preference *chosen ) {
	preference *p;
	std::map<Symbol*, rli_action> actions;
	std::map<Symbol*, rli_action>::iterator i;
	rli_action *chosen_action = NULL;
	std::list<rli_action*> state;
	std::map<production*, double> changes;
	std::map<production*, double>::iterator j;
	rli_state_id curr_state;
	rli_state_id last_state;
	...
	data = g->id.rl_info;
	gamma = a->rl_params->discount_rate->get_value();
	discount = pow( gamma, static_cast< double >( data->gap_age + data->hrl_age + 1 ) );	
	using_gaps = ( a->rl_params->temporal_extension->get_value() == soar_module::on );

	assert(chosen);
	if (g->id.operator_slot->preferences[ NUMERIC_INDIFFERENT_PREFERENCE_TYPE ]) {
		/* This is an "RL state" */
		
		// notify of gap closure
		if ( data->gap_age && using_gaps && a->sysparams[ TRACE_RL_SYSPARAM ] ) {
			char buf[256];
			SNPRINTF( buf, 254, "gap ended (%c%llu)", g->id.name_letter, static_cast<unsigned long long>(g->id.name_number) );
	
			print( a, buf );
			xml_generate_warning( a, buf );
		}
		
		for (p = g->id.operator_slot->preferences[ NUMERIC_INDIFFERENT_PREFERENCE_TYPE ]; p; p = p->next) {
			actions[p->value].insert(p->inst->prod);
		}
		for (i = actions.begin(); i != actions.end(); ++i) {
			state.push_back(&(i->second));
			if (chosen && i->first == chosen->value) {
				chosen_action = &(i->second);
			}
		}
		
		last_state = a->dp->get_state(data->trace);
		curr_state = a->dp->update_state(data->trace, state, g->id.rl_info->reward);
		
		/*
		 * Perform the update on the last state, because that's the state
		 * whose transition model was just updated.
		 */
		if (last_state > 0) {
			a->dp->dp_update(last_state, discount, changes);
			for (j = changes.begin(); j != changes.end(); ++j) {
				rl_set_qvalue(a, j->first, j->second);
			}
		}
				
		if (chosen_action) {
			a->dp->set_action(data->trace, chosen_action);
		} else {
			/* agent has taken an action that doesn't have a Q-value */
			a->dp->set_null_action(data->trace);
		}
...
void rl_update_terminal(agent *a, Symbol *g) {
	rl_data *data;
	double gamma, discount;
	rli_state_id last_state;
	std::map<production*, double> changes;
	std::map<production*, double>::iterator i;
	...
	last_state = a->dp->get_state(data->trace);
	a->dp->update_state_terminal(data->trace, g->id.rl_info->reward);
	a->dp->dp_update(last_state, discount, changes);
	for (i = changes.begin(); i != changes.end(); ++i) {
		rl_set_qvalue(a, i->first, i->second);
	}
}