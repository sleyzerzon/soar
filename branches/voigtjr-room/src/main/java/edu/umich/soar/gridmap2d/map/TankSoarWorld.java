...
public class TankSoarWorld implements World {
	private static Log logger = LogFactory.getLog(TankSoarWorld.class);

	private TankSoarMap map;
	private PlayersManager<Tank, TankCommand> players = new PlayersManager<Tank, TankCommand>();
	private int maxMissilePacks;
	private List<String> stopMessages = new ArrayList<String>();
	private final Simulation sim;
	
	public TankSoarWorld(Simulation sim) {
		this.maxMissilePacks = sim.getConfig().tanksoarConfig().max_missile_packs;
		this.sim = sim;
	}
	
	@Override
	public void setAndResetMap(String mapPath) {
		TankSoarMap newMap = TankSoarMap.generateInstance(mapPath, sim.getConfig().tanksoarConfig().max_sound_distance);
		if (newMap == null) {
			return;
		}
		map = newMap;
	...
	private void resetState() {
		...
		// Spawn missile packs
		while (map.numberMissilePacks() < sim.getConfig().tanksoarConfig().max_missile_packs) {
	...
	private void readds() {
		...
		for (Tank tank : players.getAll()) {
			...
			tank.getState().setLocation(location);
		...
		for (Tank tank : players.getAll()) {
			...
			if (players.numberOfPlayers() < 2) {
				...
				for (Tank otherTank : players.getAll()) {
					...
					int [] playerLoc = tank.getState().getLocation();
					int [] otherLoc = otherTank.getState().getLocation();
				...
				if (distance > sim.getConfig().tanksoarConfig().max_sound_distance) {
	...
	public void update(int worldCount) {
		...
		for (Tank tank : players.getAll()) {
			...
			TankCommand command = tank.getCommand();
			if (command == null) {
				sim.stop();
				return;
			}
			players.setCommand(tank, command);
			WorldUtil.checkStopSim(sim, stopMessages, command.isStopSim(), tank);
		}
		...
		for (Tank tank : players.getAll()) {
			...
			TankCommand playerMove = players.getCommand(tank);
			...
			int [] oldLocation = tank.getState().getLocation();
			...
			// Check shields
			if (playerMove.isShields()) {
				state.setShieldsUp(playerMove.getShields());
			}
			
			// Radar
			if (playerMove.isRadarSwitch()) {
				state.setRadarSwitch(playerMove.getRadarSwitch());
			}
			if (playerMove.isRadarPower()) {
				state.setRadarPower(playerMove.getRadarPower());
			}
			...
			Direction.translate(newLocation, playerMove.getMoveDirection());
			...
			if (map.getCell(newLocation).hasObjectWithProperty(Names.kPropertyBlock)) {
				...
				// Cancel the move
				playerMove = TankCommand.cancelMove(playerMove);
				players.setCommand(tank, playerMove);
				newLocations.put(tank, tank.getState().getLocation());
				
				// take damage
				String name = map.getCell(newLocation).getFirstObjectWithProperty(Names.kPropertyBlock).getProperty("name");
				state.adjustHealth(sim.getConfig().tanksoarConfig().collision_penalty, name);
			...
			TankCommand otherMove = players.getCommand(other);
			if (!otherMove.isMove()) {
			...
			if (playerMove.getMoveDirection() != otherMove.getMoveDirection().backward()) {
			...
			state.adjustHealth(sim.getConfig().tanksoarConfig().collision_penalty, "cross collision " + other);
			// Getting rammed on a charger is deadly
			if (map.getCell(tank.getState().getLocation()).hasObjectWithProperty(Names.kPropertyCharger)) {
				state.adjustHealth(state.getHealth() * -1, "hit on charger");
			}
			
			TankState otherState = other.getState();
			otherState.adjustHealth(sim.getConfig().tanksoarConfig().collision_penalty, "cross collision " + tank);
			// Getting rammed on a charger is deadly
			if (map.getCell(other.getState().getLocation()).hasObjectWithProperty(Names.kPropertyCharger)) {
			...
			playerMove = TankCommand.cancelMove(playerMove);
			players.setCommand(tank, playerMove);
			otherMove = TankCommand.cancelMove(otherMove);
			players.setCommand(other, otherMove);
			
			// store new locations
			newLocations.put(tank, tank.getState().getLocation());
			newLocations.put(other, other.getState().getLocation());
		}
		...
		for (Tank tank : players.getAll()) {
			...
			if (state.getShieldsUp()) {
				if (state.getEnergy() > 0) {
					state.adjustEnergy(sim.getConfig().tanksoarConfig().shield_energy_usage, "shields");
				} else {
		...
		for (List<Tank> collision : collisionMap.values()) {
			...
			if (collision.size() > 1) {
				...
				damage *= sim.getConfig().tanksoarConfig().collision_penalty;
		...
		for (Tank tank : movedTanks) {
			...
			TankCommand move = players.getCommand(tank);
			for (CellObject missile : missiles) {
				if (move.getMoveDirection() == Direction.valueOf(missile.getProperty(Names.kPropertyDirection)).backward()) {
		...
		for (Tank tank : firedTanks) {
			int [] missileLoc = tank.getState().getLocation();
		...
		for (Tank tank : killedTanks.keySet()) {
			// apply points
			Players.adjustPoints(tank, tank.getState().getPoints(), sim.getConfig().tanksoarConfig().frag_penalty, "fragged");
			assert killedTanks.containsKey(tank);
			for (Tank assailant : killedTanks.get(tank)) {
				if (assailant.equals(tank)) {
					continue;
				}
				Players.adjustPoints(assailant, assailant.getState().getPoints(), sim.getConfig().tanksoarConfig().frag_award, "fragged " + tank);
			}
		...
		if ((missileReset >= sim.getConfig().tanksoarConfig().missile_reset_threshold) && (killedTanks.size() == 0)) {
	...
	private void chargeUp(Tank tank, int [] location) {
		...
		for (CellObject charger : chargers) {
			if (charger.hasProperty(Names.kPropertyHealth)) {
				...
				if (state.getHealth() < sim.getConfig().tanksoarConfig().max_health) {
			...
			if (charger.hasProperty(Names.kPropertyEnergy)) {
				...
				if (state.getEnergy() < sim.getConfig().tanksoarConfig().max_energy) {
	...
	private void addCharger(boolean health) {
		...
		if (location == null) {
			sim.error("no available location for charger spawn");
			return;
		}
	...
	private void spawnMissilePack(TankSoarMap theMap, boolean force) {
		if (force || (Simulation.random.nextInt(100) < sim.getConfig().tanksoarConfig().missile_pack_respawn_chance)) {
	...
	public void missileHit(Tank tank, CellObject missile) {
		...
		Players.adjustPoints(tank, tank.getState().getPoints(), sim.getConfig().tanksoarConfig().missile_hit_penalty, owner + "-" + missile.getProperty("missile-id"));
		Tank other = players.get(owner);
		// can be null if the player was deleted after he fired but before the missile hit
		if (other != null) {
			Players.adjustPoints(other, other.getState().getPoints(), sim.getConfig().tanksoarConfig().missile_hit_award, owner + "-" + missile.getProperty("missile-id"));
		}
	...
	private void doMoveCollisions(Tank player, 
		...
		if (collision != null) {
			...
			if (collision.size() == 1) {
				...
				if (players.getCommand(other).isMove()) {
			...
			// cancel my move
			if (players.getCommand(player).isMove()) {
	...
	private void cancelMove(Tank tank, Map<Tank, int []> newLocations, 
			List<Tank> movedTanks) {
		TankCommand move = players.getCommand(tank);
		players.setCommand(tank, TankCommand.cancelMove(move));
