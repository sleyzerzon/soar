...
/********************************************************************
* @file gski_inputworkingmemory.h 
*********************************************************************
...
namespace gSKI {
   ...
   public:
       ...
       *         interface
       *
       * This method returns a pointer to the agent that owns this InputWorkingMemory
       * object.
       *
       ...
       *
       * This method adds a Wme to an agent's working memory using the most type safe
       * IWMObject.
       */
      // TODO: Fully document these function
      IWme* AddWme(IWMObject* obj,
                   const ISymbol* attribute,
                   const ISymbol* value);
       ...
       * @param object a pointer to the desired object pointer
       * @param err Pointer to client-owned error structure.  If the pointer
       *               is not 0 this structure is filled with extended error
       *               information.  If it is 0 (the default) extended error
       *               information is not returned.
      ...
      /**
       * @brief Used to obtain a InputWMObject from a IWMObject pointer
       *
       */
      InputWMObject* GetOrCreateObjectFromInterface(IWMObject* iwmobj);

      /**
       * @brief Used to create or retrieve an InputWMObject from an id symbol
       */
      InputWMObject* GetOrCreateObjectFromSymbol(const ISymbol* idsym);

      /**
       * @brief Used to creat or retrieve an InputWme from an IWme*
       */
      InputWme* GetOrCreateWmeFromInterface(IWme* wme);

      /**
       * @brief Used to create an InputWMObject from an IWMObject interface pointer
       */
      InputWMObject* CopyObjectFromInterface(const IWMObject* obj);
   ...
   private:
      ...
      typedef tWMObjMap::iterator tWMObjMapItr;

      tWMObjMap m_wmobjectmap;
      InputWMObject* m_rootInputObject;

      typedef std::map<unsigned long, InputWme*> tWmeMap;
