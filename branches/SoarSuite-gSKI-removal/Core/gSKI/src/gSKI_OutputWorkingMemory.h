...
/********************************************************************
* @file gski_outputworkingmemory.h 
*********************************************************************
* created:	   6/13/2002   14:54
*
* purpose: 
*********************************************************************/
#ifndef GSKI_OUTPUTWORKINGMEMORY_H
#define GSKI_OUTPUTWORKINGMEMORY_H
...
namespace gSKI {
  ...
  public:
    ...
    virtual ~OutputWorkingMemory();
      ...
      *         interface
      *
      * This method returns a pointer to the agent that owns this OutputWorkingMemory
      * object.
      *
    ...
    ISymbolFactory* GetSymbolFactory(Error* err = 0);
       ...
       * @param object a pointer to the wmo pointer.
       * @param err Pointer to client-owned error structure.  If the pointer
       *               is not 0 this structure is filled with extended error
       *               information.  If it is 0 (the default) extended error
       *               information is not returned.
      ...
      */
      void GetOutputRootObject(OutputWMObject** rootObect);
   ...
   private:
       ...
       */
      wme* GetOutputWme(Symbol* id, Symbol* attribute, Symbol* value);
       ...
       */
      OutputWMObject* GetOrCreateWMObject(Symbol* idSymbol);

      /**
       * @brief Used to obtain a InputWMObject from a IWMObject pointer
       *
       */
      OutputWMObject* GetOrCreateObjectFromInterface(IWMObject* iwmobj);

      /**
       * @brief Used to create (or retrieve already existing) Wme given a raw
       *         wme pointer
       */
      OutputWme* GetOrCreateWme(wme* rawWme, IWMObject* valobj);
      ...
      OutputWMObject* m_rootOutputObject;

      typedef std::map<unsigned long, OutputWme*> tWmeMap;
