...
public class GridMapPanel extends JPanel implements SimEventListener {
	...
	public GridMapPanel(Adaptable app) {
		this.app = app;
...
//				map.getCell(location).setModified(false);
	...
	void drawExplosion(Graphics g, int x, int y) {
	...
	int mouthCount = 0;
	void drawEaterMouth(Graphics g, int x, int y, int x_mult, int y_mult, int cx_mult, int cy_mult){		
		g.setColor(SystemColor.control);
	    switch(++mouthCount % 8){
			case(0):{
			    g.fillPolygon(
