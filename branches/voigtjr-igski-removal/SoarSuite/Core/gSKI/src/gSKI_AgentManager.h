...
/********************************************************************
* @file gski_agentmanager.h
*********************************************************************
* created:	   6/27/2002   10:44
*
* purpose: 
*********************************************************************/
#ifndef GSKI_AGENTMANGER_H
...
namespace gSKI
{
   ...
   */
   ...
   public:
      ...
      *  manipulation by the client.  Furthermore it can be run by
      *  associating it with a thread group (or by calling AgentManager::Run
      ...
      *
      */
      void RemoveAgentByName(const char* name, Error* err = 0);
      ...
      */
      tIAgentIterator* GetAgentIterator(Error* err = 0);
       ...
       *               information is not returned.
       */
      void            AddAllAgentsToRunList(Error* err = 0);
       ...
       */
      bool InterruptAll(egSKIStopLocation    stopLoc, 
                        Error*               err   = 0);
       ...
       *
       */
      void ClearAllInterrupts(Error* err = 0);
      ...
      */
      bool ReinitializeAll(Error* err = 0);
      ...
      */
      ...
      //////////////////////////////////////////////////////////////////////////////////////////
      // Need this for initialization (agent calls this method)

      /** 
       * @brief Called back by the agent before and after it is reinitialized
       *
     ...
     //}
      ...
      public: 
         ...
         /** Used to store the agents (tAgentMap::t is the actual container) */
         typedef FwdContainerType< std::map<std::string, Agent *> > tAgentMap;
      ...
      private:
        ...
        /** 
         * @brief Event notifier for agent events
         */
         class AgentNotifier {
         public:
         ...
         */
         ...
         /**  
          * @brief Run listener used to remove agent's at the
          *          end of a phase
          */
         class AgentRunCompletedListener: public IRunListener
         {
         public:
            void SetAgentManager(AgentManager* am) { m_am = am; }
            /** 
      ...
      private:
         ...
         tAgentMap::t              m_agents;           /**< The list of agents owned by this manager. */

         tAgentListenerManager     m_agentListeners;   /**< The list of listeners for agent events */
         
			Kernel*                   m_kernel;

         AgentRunCompletedListener m_runCompleteListener;    /**< Listens for agent done running so it can be destroyed */
