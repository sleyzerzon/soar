...
bool KernelSML::HandleDestroyAgent(AgentSML* pAgentSML, char const* /*pCommandName*/, Connection* pConnection, AnalyzeXML* /*pIncoming*/, soarxml::ElementXML* /*pResponse*/)
{
	...
	//std::cout << std::endl << "HandleDestroyAgent firing event, connection " << pConnection << ", agent " << pAgentSML->GetName() << std::endl;
	FireAgentEvent( pAgentSML, smlEVENT_BEFORE_AGENT_DESTROYED );

	//std::cout << std::endl << "HandleDestroyAgent deleting agent, connection " << pConnection << ", agent " << pAgentSML->GetName() << std::endl;
