...
void AgentListener::OnEvent(smlAgentEventId eventID, AgentSML* pAgentSML)
{
	...
	{
		if ( eventID == smlEVENT_BEFORE_AGENT_DESTROYED )
		{
			std::cout << "not firing smlEVENT_BEFORE_AGENT_DESTROYED" << std::endl;
		}
	...
	if ( eventID == smlEVENT_BEFORE_AGENT_DESTROYED )
	{
		std::cout << "firing smlEVENT_BEFORE_AGENT_DESTROYED for agent " << pAgentSML->GetName() << ", on connection " << pConnection << std::endl;
	}
