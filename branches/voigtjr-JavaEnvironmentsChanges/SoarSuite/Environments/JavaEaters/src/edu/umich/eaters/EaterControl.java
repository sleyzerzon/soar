...
public class EaterControl extends SimulationControl {
	...
	protected EatersArgumentContainer commandLine = null;
	...
	/**
	 * Creates a new instance of EaterControl, using the argument container.
	 */
	public EaterControl(EatersArgumentContainer commandLine) {
		this.commandLine = commandLine;
		
		logger = new Logger(commandLine.logfile, commandLine.clearlog, commandLine.loglevel);
		logger.log(commandLine.toString() + "\n");
		...
		if (commandLine.map == null) {
			...
			loadMap(new File(commandLine.map));
		}
	}
...
//		kernel.RunAllAgents(50, smlRunStepSize.sml_UNTIL_OUTPUT);
	}
	...
	public void soarStopEvent(int eventID, Object data, Kernel kernel) {
		if (!running) {
			...
			if (commandLine.run && commandLine.nowindows)
				quitSimulation();
		}
	}
	...
	public void updateEventHandler(int eventID, Object data, Kernel kernel,
		...
		if (!running) {
			...
			if (soarShouldStop || simShouldStop()) {
				...
				if (simShouldStop())
				{
					this.fireSimEndedNotification("All the food has been successfully eaten!");
					logger.log("\n---Final Scores---\n");
