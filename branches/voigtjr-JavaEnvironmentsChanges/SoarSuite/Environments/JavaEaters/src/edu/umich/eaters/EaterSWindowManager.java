...
public class EaterSWindowManager extends SWindowManager {
	public EaterSWindowManager(SimulationControl sc, boolean agentWindowsOn,
			boolean mapWindowOn, boolean controlPanelOn, boolean disablePopups) {
		super(sc, agentWindowsOn, mapWindowOn, controlPanelOn, disablePopups,
				"Eaters", "Eaters", "seater", "Seater files (*.seater)");
	}
	...
	public void openAgentView(SoarAgent agent) {
		...
		if (curr == null) {
			if (mySC instanceof EaterControl) {
				aw = new EaterWindow(myDisplay, (EaterControl) mySC,
						(Eater) agent);
				agentWindows.add(aw);
			}
		} else
